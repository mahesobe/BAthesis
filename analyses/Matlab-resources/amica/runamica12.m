...
%   use_queue           name of queue to use, def=all.q. Use 'use_queue',0
%   to run locally
...
function [weights,sphere,mods] = runamica12(dat,varargin)
...
%
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'use_queue') || strcmp(Keyword,'qsub')
       ...
       elseif strcmp(Keyword,'qsubname')
           qsubName = Value;
...
elseif isstruct(dat)
    ...
    if strcmp(dat(end-3:end),'.set')
        ...
        file = [EEG.filepath '/' EEG.datfile]
    else
        if chans < 0
            error('Need num_chans keyword argument with number of channels, unknown file type');
        else
            file = fullfile(pwd,dat);
            fid = fopen(file,'r');
            tmpdat = fread(fid,[chans inf],'float');
            fclose(fid);
            frames = size(tmpdat,2);
            clear tmpdat;
        end
    end
end

%added bz behinger to fix a problem in the script
if pcakeep < 0
        pcakeep = chans;
end
...
fprintf(fid,'print_debug 0\n');
fprintf(fid,'min_dll 1.00000e-06\n');
fprintf(fid,'min_grad_norm 1.00000e-06\n');
fprintf(fid,'mineig 1e-15\n');
fclose(fid);
...
%         % use ORTE
%         fprintf(fid,...
%             [MPI_BIN 'mpirun ' AMBIN '-ompi ' outdir 'input.param']);
%     end
    fprintf(fid,[AMBIN ' ' outdir 'input.param']);
    fclose(fid);
%     system(['ssh `qconf -ss` -n qsub ' outdir 'qsub.sh > ' outdir
%     'lastqsubid']);
%     system(['qsub -l mem_free=40G -N ' qsubName ' ' outdir 'qsub.sh']);use this for ramsuaer

    system(['qsub -l mem=8G,h=!ramsauer.ikw.uni-osnabrueck.de -pe default ' num2str(max_threads) ' -N ' qsubName ' ' outdir 'qsub.sh']);

%     system(['qsub -l mem=' num2str(ceil(8/max_threads)) 'G -pe default ' num2str(max_threads) ' -N ' qsubName ' ' outdir 'qsub.sh']);
...
%     fprintf(['   $ watch tail -n 25 %sqsub.sh.o' int2str(qsubid) '\n'],outdir);
%     disp(['or in Matlab:']);
%     fprintf(['   >> system(''watch tail -n 25 %sqsub.sh.o' int2str(qsubid) ''') \n'],outdir);
